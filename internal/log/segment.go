...
func newSegment(dir string, baseOffset uint64, c Config) (*segment, error) {
	...
	if off, _, err := segment.index.Read(-1); err != nil {
		...
		//set the next record's offset if we have an entry.
...
func nearestMultiple(limit, multiple uint64) uint64 {
	if limit >= 0 {
		return (limit / multiple) * multiple
	}

	return ((limit - multiple + 1) / multiple) * multiple
}
